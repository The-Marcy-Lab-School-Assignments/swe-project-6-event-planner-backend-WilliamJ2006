...
module.exports.list = async () => {
  const query = `SELECT events.*, username, COUNT(rsvps.user_id) AS rsvp_count FROM events JOIN users ON events.user_id = users.user_id LEFT JOIN rsvps ON events.event_id = rsvps.event_id GROUP BY events.event_id, users.user_id ORDER BY events.event_id`;
  const { rows } = await pool.query(query);
  return rows;
};

module.exports.listByUser = async (user_id) => {
  const query = `SELECT * FROM events WHERE events.user_id = $1 ORDER BY events.event_id`;
